/* KHALID CLOUD SYNC v1.1 — sincronizacao sem duplicacoes. */
(function(){
  ...
  function fp(k,x){
    ...
    if(k==='sistemaKhalidInspecoes')return x.id&&/^[0-9a-f-]{36}$/i.test(String(x.id))?'i|'+x.id:'i|'+clean(x.condominio_nome||x.condominio)+'|'+iso(x.data_inspecao||x.data)+'|'+clean(x.inspetor||x.supervisor)+'|'+clean(x.resultado||x.status);
    if(k==='sistemaKhalidOcorrencias')return x.id&&/^[0-9a-f-]{36}$/i.test(String(x.id))?'o|'+x.id:'o|'+clean(x.condominio_nome||x.condominio)+'|'+iso(x.data_ocorrencia||x.data)+'|'+clean(x.titulo||x.assunto)+'|'+clean(x.descricao||x.relato);
    if(k==='khalidTreinamentos')return x.id&&/^[0-9a-f-]{36}$/i.test(String(x.id))?'t|'+x.id:'t|'+clean(x.colaborador_nome||x.colaborador||x.nome)+'|'+clean(x.curso||x.tipo)+'|'+clean(x.data_treinamento||x.data)+'|'+clean(x.condominio_nome||x.condominio);
  ...
  function toCloud(k,x){
    ...
    if(k==='sistemaKhalidInspecoes')return {condominio_nome:x.condominio_nome||x.condominio||'',data_inspecao:iso(x.data_inspecao||x.data||x.created_at),inspetor:x.inspetor||x.supervisor||'Khalid',resultado:x.resultado||x.status||'',observacoes:x.observacoes||'',itens:x.itens||{}};
  ...
  async function syncKey(k){
    ...
    const cloudByFp=new Map(cloud.map(x=>[fp(k,x),x]));
    const localUnique=new Map();
    for(const x of local)localUnique.set(fp(k,x),x);
    for(const [key,x] of localUnique){if(cloudByFp.has(key))continue;try{await req(table,'POST',toCloud(k,x))}catch(e){console.warn('KHALID CLOUD upload',table,e)}}
    const fresh=await req(table,'GET'); const merged=new Map();
    for(const x of fresh)merged.set(fp(k,x),fromCloud(k,x));
    for(const x of localUnique.values())if(!merged.has(fp(k,x)))merged.set(fp(k,x),x);
